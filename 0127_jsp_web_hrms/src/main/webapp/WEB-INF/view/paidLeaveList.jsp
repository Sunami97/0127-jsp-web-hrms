...
.paid-leave-table th, .paid-leave-table td {
...
<body>
		...
		<table>
			...
			<tbody>
				...
				<c:forEach var="leave" items="${paidLeavePage.content}">
					<tr
						onclick="goToDetail(${leave.leaveId}, ${paidLeavePage.currentPage})">
						<td>${leave.leaveId}</td>
						<td>${leave.name}</td>
	...
	<c:if test="${paidLeavePage.hasPaidLeaves()}">
		<div class="pagination">
			...
			<c:forEach var="pNo" begin="${paidLeavePage.startPage}"
				end="${paidLeavePage.endPage}">
