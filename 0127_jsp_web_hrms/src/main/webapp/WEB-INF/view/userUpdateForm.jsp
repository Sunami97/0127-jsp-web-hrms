		...
		<div>
			<div class="container">
				...
				* 名前 <br> <input type="text" name="name" value="${user.name}"><br>
				<br> * メール<br> <input type="text" name="email"
					value="${user.email}"><br> <br> * 連絡先<br> <input
					type="text" name="phone" value="${user.phone}"><br> <br>
				* 職位 <br> <select name="position">
					<c:forEach var="pos" items="${positions}">
						<option value="${pos}"
							<c:if test="${user.position eq pos}">selected</c:if>>${pos}</option>
					</c:forEach>
				</select><br> <br>
				<!-- 部署 -->
				* 部署 <br> <select name="departmentId">
					<c:forEach var="entry" items="${departments}">
						<option value="${entry.key}"
							<c:if test="${user.departmentId eq entry.key}">selected</c:if>>${entry.value}</option>
					</c:forEach>
				</select><br> <br>
				<!-- 管理者権限 -->
				* 管理者 権限<br> N <input type="radio" name="isAdmin" value="N"
					<c:if test="${user.isAdmin eq 'N'}">checked</c:if> /> Y <input
					type="radio" name="isAdmin" value="Y"
					<c:if test="${user.isAdmin eq 'Y'}">checked</c:if> /> <br> <br>
				<!-- 状態 -->
				* 状態 <br> <select name="empStatus">
					<c:forEach var="emp" items="${empStatuss}">
						<option value="${emp}"
							<c:if test="${user.empStatus eq emp}">selected</c:if>>${emp}</option>
					</c:forEach>
				</select> <br> <br>
				...
				<c:if test="${empty user.retireDate }">
					<button type="submit" class="admin-button" onclick="fire()">退社</button>
				</c:if>
				<button type="submit" class="admin-button" onclick="update()">修正</button>
			</div>
		</div>
	</form>
